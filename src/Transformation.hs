  ...
  case f of
    ...
    (Not a)       -> do
      ...
      if l == LTrue then return (s, Iff (Lit s) (Not f'))
                    else return (s, And (Iff (Lit s) (Not (Lit l))) f')
  where
    getFreshVar = do
      n <- get
      put (n + 1)
    ...
    binaryOpTseitin op a b = do
      ...
      if la == LTrue
         then if lb == LTrue
                 ...
                 else return (s, And (Iff (Lit s) (op fa (Lit lb))) fb)
         else if lb == LTrue
                 then return (s, And (Iff (Lit s) (op (Lit la) fb)) fa)
                 else return (s, And (Iff (Lit s) (op (Lit la) (Lit lb))) (And fa fb))
